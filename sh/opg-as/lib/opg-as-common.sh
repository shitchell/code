...
function generate-customization-pattern {
    ...
    cust_pattern="AS9?[_-](CUS[A-Za-z]{,2}?[_-]${cust_type}|${cust_type}[_-]CUS[A-Za-z]{,2}?)[_-]${cust_nmbr}|CUS[A-Za-z]{,2}?[_-]AS9?[_-]${cust_type}[_-]${cust_nmbr}"
...
function get-metadata-by-file-from-xml() {
    ...
    awk -v filepath="${filepath}" -v name_only="${NAME_ONLY:-0}" \
        ...
        {
            if (in_object == 1 && $0 ~ "<timestamp>") {
                timestamp=gensub(/.*<timestamp>(.*?)<\/timestamp>.*/, "\\1", "g");
            }
        }
        # ...mode...
        {
            if (in_object == 1 && $0 ~ "<mode>") {
                mode=gensub(/.*<mode>(.*?)<\/mode>.*/, "\\1", "g");
            }
        }
        # ...and checksum
        {
            if (in_object == 1 && $0 ~ "<checksum>") {
