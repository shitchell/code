# Integration tests for provider.clip.xsel — X11 / XWayland.
#
...
@test "provider.clip.xsel probe: caps with an X display, score 0 without" {
  run provider.clip.xsel probe
  [ "$status" -eq 0 ]
  if [[ -n "$DISPLAY" ]]; then
    [[ "$output" == *"get:plain"* ]]
    [[ "$output" == *"set:plain"* ]]
...
@test "provider.clip.xsel round-trips plain text (skips without X / if not functional)" {
  [[ -n "$DISPLAY" ]] || skip "no DISPLAY (xsel needs X/XWayland)"
  export VAL="rt-xsel-$$-$RANDOM"
  run bash -c 'printf "%s" "$VAL" | timeout 30 provider.clip.xsel set plain'
  if [ "$status" -ne 0 ]; then skip "xsel set not functional (status $status)"; fi
  run timeout 30 provider.clip.xsel get plain
