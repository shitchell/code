...
@test "picks highest-score provider that supports the op:type" {
  make_provider provider.clip.low  30 "get:plain set:plain" "LOWVAL"
  make_provider provider.clip.high 80 "get:plain set:plain" "HIGHVAL"
...
@test "skips a higher-score provider that lacks the requested capability" {
  make_provider provider.clip.rich 90 "get:plain set:plain" "PLAIN90"   # no get:rich
  make_provider provider.clip.has  40 "get:plain get:rich"  "RICH40"
  run clip::dispatch get rich
  [ "$status" -eq 0 ]; [ "$output" = "RICH40" ]
}

@test "image with no provider errors with hint and exit 3" {
  make_provider provider.clip.txt 50 "get:plain set:plain" "X"
...
EOF
  chmod +x "$dir/provider.clip.aaa"          # sorts first; drains stdin in probe
  export PATH="$dir:$PATH"
  make_provider provider.clip.zzz 90 "get:plain set:plain" "ZZZVAL"   # sorts last
  run clip::dispatch get plain
  [ "$status" -eq 0 ]
  [ "$output" = "ZZZVAL" ]
}
...
@test "both providers present: wl (70) wins plain over gpaste (50)" {
  ...
  make_provider provider.clip.wl     70 "get:plain set:plain get:rich set:rich get:image set:image" "WLVAL"
  make_provider provider.clip.gpaste 50 "get:plain set:plain" "GPVAL"
  run clip::dispatch get plain
  [ "$status" -eq 0 ]
  [ "$output" = "WLVAL" ]
}
...
@test "both providers present: rich routes to wl (only wl offers it)" {
  make_provider provider.clip.wl     70 "get:plain set:plain get:rich set:rich get:image set:image" "WLRICH"
  make_provider provider.clip.gpaste 50 "get:plain set:plain" "GPVAL"
  run clip::dispatch get rich
  [ "$status" -eq 0 ]
  [ "$output" = "WLRICH" ]
...
@test "both providers present: image routes to wl (only wl offers it)" {
  make_provider provider.clip.wl     70 "get:plain set:plain get:rich set:rich get:image set:image" "WLIMG"
  make_provider provider.clip.gpaste 50 "get:plain set:plain" "GPVAL"
  run clip::dispatch get image
  [ "$status" -eq 0 ]
  [ "$output" = "WLIMG" ]
...
@test "gpaste-only box (wl scores 0): gpaste still wins plain" {
  # On a machine with gpaste but no wl-* binaries, provider.clip.wl probes score 0 and is
  # ineligible, so gpaste is selected for plain.
  make_provider provider.clip.wl     0  "" ""
  make_provider provider.clip.gpaste 50 "get:plain set:plain" "GPVAL"
  run clip::dispatch get plain
  [ "$status" -eq 0 ]
  [ "$output" = "GPVAL" ]
