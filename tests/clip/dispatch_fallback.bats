...
setup() {
  ...
  # Isolate PATH so clip::_providers (compgen -c 'provider.clip.') sees ONLY the fakes
  # this file creates, not the real provider.clip.* in ~/code/sh/bin. make_* helpers
...
@test "chosen provider times out -> falls back to next and returns its output" {
  ...
  make_hanging_provider provider.clip.hang 80 "get:plain set:plain"
  make_provider         provider.clip.ok   50 "get:plain set:plain" "OKVAL"
...
@test "chosen provider exits non-zero -> falls back to next" {
  make_failing_provider provider.clip.fail 80 "get:plain set:plain"
  make_provider         provider.clip.ok   50 "get:plain set:plain" "OKVAL"
  run clip::dispatch get plain
  [ "$status" -eq 0 ]
  [ "$output" = "OKVAL" ]
...
@test "set payload reaches the fallback provider intact after first fails" {
  make_failing_provider provider.clip.fail 80 "get:plain set:plain"
  make_provider         provider.clip.ok   50 "get:plain set:plain" ""   # writes stdin to sink
  run bash -c "printf '%s' 'PAYLOAD-abc-123' | { source '$BATS_TEST_DIRNAME/../../sh/lib/clip.sh'; clip::dispatch set plain; }"
  [ "$status" -eq 0 ]
  [ "$(cat "$BATS_TEST_TMPDIR/provider.clip.ok.sink")" = "PAYLOAD-abc-123" ]
}

@test "all capable providers fail -> exit 3 with hint" {
  make_failing_provider provider.clip.f1 80 "get:plain set:plain"
  make_failing_provider provider.clip.f2 50 "get:plain set:plain"
  run clip::dispatch get plain
  [ "$status" -eq 3 ]
...
@test "get output of a failed provider is not emitted before falling back" {
  ...
  cat > "$dir/provider.clip.partial" <<'EOF'
#!/bin/bash
case "$1" in
...
EOF
  chmod +x "$dir/provider.clip.partial"
  export PATH="$dir:$PATH"
  make_provider provider.clip.ok 50 "get:plain set:plain" "CLEAN"
  run clip::dispatch get plain
  [ "$status" -eq 0 ]
  [ "$output" = "CLEAN" ]
}
