...
@test "clipout with no format emits plaintext" {
  make_provider provider.clip.g 70 "get:plain set:plain" "hello"
  run sh/bin/clipout
  [ "$status" -eq 0 ]
  [ "$output" = "hello" ]
}
...
@test "clipout --format rich returns provider rich output" {
  make_provider provider.clip.r 80 "get:plain get:rich" "<b>hi</b>"
...
@test "clipout --image dispatches a get image" {
  make_provider provider.clip.i 90 "get:plain get:image" "IMGBYTES"
